...
enum MENU {
  ...
  MENU_FLAG,
  MENU_DEXNAV,
  MENU_MIN = MENU_POKEDEX,
  MENU_MAX = MENU_DEXNAV,
};
...
static EWRAM_DATA u8 menuSelected = MENU_NONE;
...
// --BG-GFX--
static const u32 sStartMenuTiles[] = INCGFX_U32("graphics/heat_start_menu/bg.png", ".4bpp.smol");
static const u32 sStartMenuTilemap[] = INCGFX_U32("graphics/heat_start_menu/bg.bin", ".smol");
static const u32 sStartMenuTilemapSafari[] = INCGFX_U32("graphics/heat_start_menu/bg_safari.bin", ".smol");
static const u16 sStartMenuPalette[] = INCGFX_U16("graphics/heat_start_menu/bg.png", ".gbapal");
// static const u16 gStandardMenuPalette[] = INCGFX_U16("graphics/interface/std_menu.pal", ".gbapal");

//--SPRITE-GFX--
#define TAG_ICON_GFX 1234
#define TAG_ICON_PAL 0x4654

static const u32 sIconGfx[] = INCGFX_U32("graphics/heat_start_menu/icons.png", ".4bpp.smol");
static const u16 sIconPal[] = INCGFX_U16("graphics/heat_start_menu/icons.png", ".gbapal");
...
static void SpriteCB_IconDexnav(struct Sprite* sprite) {
  if (menuSelected == MENU_DEXNAV && sHeatStartMenu->flag == FLAG_VALUE_NOT_SET) {
    sHeatStartMenu->flag = FLAG_VALUE_SET;
    StartSpriteAnim(sprite, 1);
    StartSpriteAffineAnim(sprite, 1);
  } else if (menuSelected != MENU_DEXNAV) {
    StartSpriteAnim(sprite, 0);
    StartSpriteAffineAnim(sprite, 0);
  } 
}
...
void HeatStartMenu_Init(void) {
  ...
  if (GetSafariZoneFlag() == FALSE) { 
    if (FlagGet(FLAG_SYS_POKENAV_GET) == FALSE && menuSelected == MENU_POKETCH) {
      menuSelected = MENU_NONE;
    }

    if (!FlagGet(DN_FLAG_DEXNAV_GET) && menuSelected == MENU_DEXNAV) {
      menuSelected = MENU_NONE;
    }
    ...
    if (menuSelected == MENU_NONE || menuSelected == MENU_POKETCH || menuSelected == MENU_SAVE) {
...
static void HeatStartMenu_CreateSprites(void) {
  ...
  if (FlagGet(DN_FLAG_DEXNAV_GET)) {
    sHeatStartMenu->spriteIdDexnav  = CreateSprite(&gSpriteIconDexnav, x, 149, 0);
  }
  if (FlagGet(FLAG_SYS_POKENAV_GET) == TRUE) {
    sHeatStartMenu->spriteIdPokedex = CreateSprite(&gSpriteIconPokedex, x-1, 8, 0);
    sHeatStartMenu->spriteIdParty   = CreateSprite(&gSpriteIconParty, x, 28, 0);
    sHeatStartMenu->spriteIdBag     = CreateSprite(&gSpriteIconBag, x, 49, 0);
    sHeatStartMenu->spriteIdPoketch = CreateSprite(&gSpriteIconPoketch, x, 70, 0);
    sHeatStartMenu->spriteIdTrainerCard = CreateSprite(&gSpriteIconTrainerCard, x, 90, 0);
    sHeatStartMenu->spriteIdSave    = CreateSprite(&gSpriteIconSave, x, 111, 0);
    sHeatStartMenu->spriteIdOptions = CreateSprite(&gSpriteIconOptions, x, 129, 0);
    return;
  } else if (FlagGet(FLAG_SYS_POKEDEX_GET) == TRUE) {
...
static void HeatStartMenu_UpdateMenuName(void) {
  ...
  switch(menuSelected) {
    ...
    case MENU_FLAG:
      ...
      break;
    case MENU_DEXNAV:
      AddTextPrinterParameterized(sHeatStartMenu->sMenuNameWindowId, 1, COMPOUND_STRING("  DexNav"), 1, 0, 0xFF, NULL);
    default:
      break;
  }
...
static void HeatStartMenu_OpenMenu(void) {
  switch (menuSelected) {
    ...
    case MENU_OPTIONS:
      ...
      break;
    case MENU_DEXNAV:
      Task_OpenDexNavFromStartMenu(FindTaskIdByFunc(Task_HeatStartMenu_HandleMainInput));
      break;
    default:
      break;
  }
